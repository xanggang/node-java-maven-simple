...
// 添加医慧相关业务
// 获取最新的包de metadata文件
Dependency.prototype.getMetadata = function() {
  return path.join(this.getVersionPath(), '/maven-metadata.xml');
}

Dependency.prototype.getLastJarPath = function() {
  if (!this.metadataXml) {
    throw new Error("最新的版本号不正确: " + JSON.stringify(this.metadataXml), this);
  }
  try {
    const lastVersions = this.metadataXml.metadata.versioning[0]
      .snapshotVersions[0].snapshotVersion[0].value[0]



    const lastVersionsPath = path.join(this.getVersionPath(),
      this.artifactId + '-' + lastVersions + '.jar')
    console.log("最后的版本号：" + lastVersionsPath);
    return lastVersionsPath
  } catch (e) {
    throw new Error("最新的版本号不正确: " + JSON.stringify(this.metadataXml), this);
  }
}
